...
void broadcast_user_availability(client_data **head)
{
    ...
    for(; iter != NULL; iter = iter->next)
    {
        ...
        {
            char output[MAX_BUFFER];
            format_message(output, "LIST_USER","LIST_USER_SEND","0","0","LIST",iter->username);
...
void inchat(client_data *connected_user, char client_ip[MAX_IP], char SIGNAL[MAX_USERNAME], char receiver[MAX_USERNAME], char sender[MAX_USERNAME], char content_type[MAX_USERNAME], char content[MAX_USERNAME])
{
    ...
    while( (read_size = recv(connected_user->socket , client_message , MAX_BUFFER , 0)) > 0 /* && strcmp(SIGNAL, "CHAT_END") != 0 */)
    {
